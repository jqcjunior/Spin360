...
export default function App() {
  ...
  const handleSelectEvent = (event: Event) => {
    setSelectedEvent(event); setActiveLead(null); setGeneratedVideo(null);
    setTotemState(event.enableLeadCapture ? 'lead' : 'recorder');
  };

  const handleLeadFormComplete = (lead: VideoLead) => { setActiveLead(lead); setTotemState('recorder'); };
  const handleRecordingSuccess = (video: VideoRecord) => { setGeneratedVideo(video); setTotemState('result'); triggerDbReload(); };
  const handleResetTotemFlow = () => { setSelectedEvent(null); setActiveLead(null); setGeneratedVideo(null); setTotemState('selection'); triggerDbReload(); };
  const handleNavigateToPublicPreview = (slug: string) => { setPublicSlug(slug); setViewMode('public_video'); };
  ...
  const handlePublicDownload = async (vid: VideoRecord) => {
    ...
    try {
      ...
      link.href = blobUrl; link.download = `Real360_${vid.slug}.mp4`;
  ...
  const handlePublicShare = async (vid: VideoRecord, channel: string) => {
    ...
    if (channel === 'whatsapp') {
      try {
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
          await navigator.share({ title: shareTitle, files: [file] }); return;
        }
      } catch {}
    ...
    if (channel === 'airdrop' || channel === 'nearby') {
      ...
      if (navigator.share) { try { await navigator.share({ title: shareTitle, url: shareUrl }); } catch {} }
      else { alert('Use Chrome (Android) ou Safari (iOS) para compartilhar.'); }
      return;
    }

    if (channel === 'link') {
      navigator.clipboard.writeText(shareUrl);
      setPbCopied(true); setTimeout(() => setPbCopied(false), 2500); return;
    }
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans flex flex-col justify-between">
      <header className="bg-slate-900 border-b border-slate-800 px-4 py-3 sticky top-0 z-50 shadow-md">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-3">
          ...
          <div className="flex items-center bg-slate-950 p-1.5 rounded-2xl border border-slate-800 text-xs font-semibold gap-1">
            <button onClick={() => { setViewMode('totem'); handleResetTotemFlow(); }}
              ...
              <MonitorPlay className="w-3.5 h-3.5" /><span>Totem de Gravação</span>
            </button>
            <button onClick={() => setViewMode('admin')}
              ...
              <LayoutDashboard className="w-3.5 h-3.5" /><span>Painel Admin</span>
            </button>
            <button onClick={() => { setViewMode('public_video'); const v = SpinDb.getVideos(); if (v.length > 0) setPublicSlug(v[0].slug); }}
              ...
              <QrCode className="w-3.5 h-3.5" /><span>Página de Download</span>
            </button>
          </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 py-8">
        {viewMode === 'totem' && (
          <div className="h-full">
            {totemState === 'selection' && (
              <div className="space-y-8 animate-fade-in">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {activeEvents.map((e) => (
                    <div key={e.id} className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-xl flex flex-col justify-between hover:border-slate-700 transition-all group p-1.5">
                      <div className="relative aspect-video rounded-2xl overflow-hidden bg-slate-950">
                        {e.coverUrl ? (
                          ...
                          <div className="w-full h-full bg-slate-950 flex items-center justify-center"><Camera className="w-8 h-8 text-slate-800" /></div>
                        )}
                        <div className="absolute top-3 left-3 bg-slate-950/80 backdrop-blur-md px-2.5 py-1 rounded-xl text-[9px] font-mono tracking-widest font-bold uppercase text-amber-500 border border-slate-800">{e.category}</div>
                      </div>
                      <div className="p-4 space-y-1.5 flex-1 mt-2">
                        ...
                        <p className="text-slate-400 text-xs line-clamp-2">{e.description || 'Evento ativo no totem.'}</p>
                      </div>
                      <div className="p-4 bg-slate-950/60 rounded-2xl mx-1.5 mb-1.5 flex justify-between items-center text-xs">
                        ...
                        <button onClick={() => handleSelectEvent(e)}
                          className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold font-mono text-[11px] tracking-wider transition-all flex items-center gap-1 shadow-lg cursor-pointer">
                          <span>GRAVAR VÍDEO</span><ArrowRight className="w-3.5 h-3.5 text-[11px]" />
                        </button>
                      </div>
                    </div>
                  ))}
                  {activeEvents.length === 0 && (
                    <div className="col-span-full bg-slate-900 border border-slate-800 p-8 rounded-3xl text-center space-y-3">
                      ...
                      <button onClick={() => setViewMode('admin')} className="bg-indigo-600 text-white px-3.5 py-2 rounded-xl text-xs font-bold font-mono cursor-pointer">Ir ao Painel Admin</button>
                    </div>
                  )}
                </div>
                  ...
                  <div className="bg-slate-900/30 p-6 rounded-3xl border border-slate-800/80">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4">
                      {allCompletedVideos.slice(-6).map((vid) => {
                        ...
                        return (
                          <div key={vid.id} style={{ borderTopColor: evt?.themeColor || '#6366f1' }}
            ...
            {totemState === 'lead' && selectedEvent && <LeadCaptureModal event={selectedEvent} onLeadCaptured={handleLeadFormComplete} onCancel={handleResetTotemFlow} />}
            {totemState === 'recorder' && selectedEvent && <CameraRecorder event={selectedEvent} lead={activeLead} onRecordingComplete={handleRecordingSuccess} onCancel={handleResetTotemFlow} />}
            {totemState === 'result' && generatedVideo && selectedEvent && <VideoPlaybackResult video={generatedVideo} event={selectedEvent} onRecordAgain={handleResetTotemFlow} />}
          </div>
        )}

        {viewMode === 'admin' && (
          <AdminDashboard onSelectEventForCapture={(evt) => { setSelectedEvent(evt); setViewMode('totem'); setTotemState(evt.enableLeadCapture ? 'lead' : 'recorder'); }} />
        )}
              ...
              return (
                <div className="space-y-4">
                  <div className="text-center space-y-1">
                    <span className="text-[10px] font-mono bg-emerald-950/80 text-emerald-400 border border-emerald-500/20 px-3 py-1 rounded-full uppercase font-bold tracking-wider">✓ Vídeo Verificado (LGPD)</span>
                  ...
                  <div className="space-y-2.5">
                    <button onClick={() => handlePublicDownload(currentVideo)}
                      className="w-full py-3.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:opacity-90 rounded-2xl text-sm font-bold font-mono text-white uppercase flex items-center justify-center gap-2 shadow-lg cursor-pointer">
                      <Download className="w-4 h-4 mb-0.5" /> Baixar Vídeo (MP4)
                    </button>
                    ...
                    <button onClick={() => handlePublicShare(currentVideo, 'nearby')}
                      className="w-full py-3 rounded-2xl bg-violet-600 hover:bg-violet-500 text-white font-bold text-sm flex items-center justify-center gap-2 cursor-pointer">
      ...
      <footer className="bg-slate-950 border-t border-slate-900 py-6 px-4">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-slate-500 font-mono">
          <div>
            ...
            <p className="text-[10px] text-slate-600">Desenvolvido pela agência ArtTech.</p>
          </div>
